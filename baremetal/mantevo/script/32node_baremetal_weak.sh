...
LOGDIR=/home/cc/exp/baremetal/log/log-"$(date +"%H:%M_%b_%d_%y")"
SCRIPT_DIR=/home/cc/exp/baremetal/mantevo/script
mkdir -p "$LOGDIR"
...
for NCPU in 24 48 96 192 384 768; do # scale up cpu
    for i in 0 1 2 3 4;do # run 10 times to get average
       case $NCPU in
           ...
           48)
              (time -p mpirun -np $NCPU --hostfile ./hostfile"$NCPU" taskset -c 0-23 ./CoMD-mpi -i 4 -j 4 -k 3 -x 288 -y 144 -z 144) > $LOGDIR/CoMD_MPI_NCPU_"$NCPU"_No_"$i".log 2>&1
           ;;
           96)
              (time -p mpirun -np $NCPU --hostfile ./hostfile"$NCPU" taskset -c 0-23 ./CoMD-mpi -i 4 -j 8 -k 3 -x 288 -y 288 -z 144) > $LOGDIR/CoMD_MPI_NCPU_"$NCPU"_No_"$i".log 2>&1
           ;;
           192)
           ...
           ;;
           384)
              (time -p mpirun -np $NCPU --hostfile ./hostfile"$NCPU" taskset -c 0-23 ./CoMD-mpi -i 8 -j 8 -k 6 -x 576 -y 288 -z 288) > $LOGDIR/CoMD_MPI_NCPU_"$NCPU"_No_"$i".log 2>&1
           ;;
           768)
              (time -p mpirun -np $NCPU --hostfile ./hostfile"$NCPU" taskset -c 0-23 ./CoMD-mpi -i 8 -j 8 -k 12 -x 576 -y 576 -z 288) > $LOGDIR/CoMD_MPI_NCPU_"$NCPU"_No_"$i".log 2>&1
           ;;
       esac
    done
done
...
for NCPU in 24 48 96 192 384 768 ; do # scale up cpu
    for i in 0 1 2 3 4; do #  run 10 times to get average
        ...
        ;;
        768)
            (time -p mpirun -np $NCPU --hostfile ./hostfile"$NCPU" taskset -c 0-23 ./miniFE.x nx=1024 ny=1024 nz=840) > $LOGDIR/miniFE_MPI_NCPU_"$NCPU"_No_"$i".log 2>&1
        ;;
        esac
    done
done
